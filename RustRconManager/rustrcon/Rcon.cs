...
namespace RustRcon
{
    ...
    public class Rcon
    {
        ...
        private BlockingCollection<Package> packages;
        ...
        public Rcon(string host, int port, string rconPassword)
        {
            ...
            this.packages = new BlockingCollection<Package>();
        ...
        public void UpdatePackage(Package package)
        {
            ...
            if (package.ID > 1)
            {
                ...
                if (validator != null) // Validator received, completing package
                {
                    ...
                    BlockingCollection<Package> new_packages = new BlockingCollection<Package>();
                    foreach (Package check in this.packages.Where(match => match.UID != package.UID && !match.Complete))//match.Time.AddSeconds(30) > DateTime.Now))
                        new_packages.Add(check);

                    this.packages = new_packages; // Discard if completed
        ...
        public Package ReadPackage(int id)
        {
            ...
            {
                BlockingCollection<Package> new_packages = new BlockingCollection<Package>();
                foreach (Package package in this.packages.Where(match => match.UID != matched.UID))
        ...
        public void Inject(Package package)
        {
            this.packages.Add(package);
        }
